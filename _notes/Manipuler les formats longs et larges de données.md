...
### [df.pivot_table()](https://pandas.pydata.org/pandas-docs/stable/user_guide/reshaping.html#pivot-tables) - pivot avec aggrégation

La méthode [[df.pivot_table()]] nous permet de définir une fonction aggrégative, comme [[df.mean()]], pour aggréger les valeurs correspondant au même **index**.
...
- `aggfunc`donne la fonction utilisée pour aggréger les valeurs. On peut passer une méthode [[dataframe]], mais aussi une fonction étrangère, comme [[np.sum()]] ou même une liste de fonctions associées à la colonne de valeurs à traiter, sous forme de dictionnaire : `{'D': np.mean, 'E': np.mean}`.

> **À noter** : Si l'on ajoute l'option `margins=True`, des [aggrégats partiels](https://pandas.pydata.org/pandas-docs/stable/user_guide/reshaping.html#adding-margins) (comme dans un tableau de comptabilité, par exemple) seront calculés et placés dans une colonne et une ligne en marge, nommées `All`.
