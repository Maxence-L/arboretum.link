...
> Si $$\overrightarrow{u} \in I\!R^n$$ alors la **Norme** ou la **Magnitude** de $$\overrightarrow{u}$$ est définie comme la longueur ou la magnitude du vecteur et can be calculée en utilisant la formule $$\vert\vec{u}\vert=\sqrt{u_{1}^{2}+u_{2}^{2}+\ldots+u_{n}^{2}}$$

[^1]: http://mathonline.wikidot.com/the-norm-of-a-vector

![distance](/assets/img/distance-vecteur.png)
...
$$d(p, q)=\sqrt{\sum_{i=1}^{d}\leftvertp_{i}-q_{i}\rightvert^{2}}$$
...
$$d_{k}(p, q)=\sqrt[k]{\sum_{i=1}^{d}\leftvertp_{i}-q_{i}\rightvert^{k}}=\left(\sum_{i=1}^{d}\leftvertp_{i}-q_{i}\rightvert^{k}\right)^{1 / k}$$
...
![vecteur unité](/assets/img/vecteur-unite.png)
...
![norme L2](/assets/img/norme-L2.png)

Cela permet de comparer les vecteurs entre eux sur la base de leur direction, et de calculer leur [[similarité cosinus]], par exemple.
